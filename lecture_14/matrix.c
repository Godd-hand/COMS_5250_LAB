...
double power_iteration(const matrix* A, vector* v, double tol, int max_iters) {
    ...
    for (int i = 1; i <= v->size; i++) {
        vgetp(v, i) = vgetp(v, i) / norm_v;
    }

    vector Av = matrix_vector_mult(A, v);
    double lambda_prev = vector_dot_mult(v, &Av);
    free_vector(&Av);
    double lambda_curr = lambda_prev;
    ...
    for (int k = 1; k <= max_iters; k++) {
        ...
        for (int i = 1; i <= v->size; i++) {
            vgetp(v, i) = vget(w, i) / norm_w;
        }
        
        vector Av_new = matrix_vector_mult(A, v);
        lambda_curr = vector_dot_mult(v, &Av_new);
...
double shifted_inverse_iteration(const matrix* A, vector* v, double mu, double tol, int max_iters) {
    ...
    matrix I = matrix_identity(A->rows);
    matrix mu_I = matrix_scalar_mult(&I, mu);
    matrix M = matrix_sub(A, &mu_I); // M = A - mu*I
...
double rayleigh_quotient_iteration(const matrix* A, vector* v, double tol, int max_iters) {
    ...
    for (int k = 1; k <= max_iters; k++) {
        matrix lam_I = matrix_scalar_mult(&I, lambda_prev);
        matrix M = matrix_sub(A, &lam_I); // M = A - lambda_prev*I
